...
somRandomInit <- function(somgrid,data) {
  data <- as.matrix(data)
  if(nrow(data)>=nrow(somgrid$pts)) {
    data[sample(1:nrow(data),size=nrow(somgrid$pts)),]
  } else {
    data[sample(1:nrow(data),size=nrow(somgrid$pts),replace=TRUE),]
  }
}
...
secondBmu <- function(prototypes,data) {
  ...
  matrix(.C("second_bmu",as.double(prototypes),as.integer(nrow(prototypes)),as.double(data),as.integer(nrow(data)),as.integer(ncol(prototypes)),clusters=integer(2*nrow(data)),PACKAGE="yasomi")$clusters,ncol=2)
}
...
batchsom <- function(somgrid,data,prototypes=somPCAInit(somgrid,data),
  ...
  batchsom.lowlevel(somgrid,data,prototypes,theRule,radii,
